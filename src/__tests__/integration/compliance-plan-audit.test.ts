...
describe("Compliance Plan — audit (NFR5)", () => {
  ...
  beforeAll(async () => {
    ...
    const u = await db.user.create({ data: { id: randomUUID(), email: `cpa-${s}@example.com`, name: "cpaadmin", organizationId: org.id, role: UserRole.ORG_ADMIN, updatedAt: new Date() } });
