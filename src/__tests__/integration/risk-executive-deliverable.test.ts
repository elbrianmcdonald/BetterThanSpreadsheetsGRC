...
describe("Risk Executive Summary Deliverable (matrix-driven)", () => {
  ...
  beforeAll(async () => {
    ...
    assignee = await db.user.create({
      data: { id: randomUUID(), email: `exec-assignee-${stamp}@example.com`, name: "Assignee", organizationId: orgA.id, role: UserRole.ANALYST, updatedAt: new Date() },
    });
    otherAnalyst = await db.user.create({
      data: { id: randomUUID(), email: `exec-other-${stamp}@example.com`, name: "Other", organizationId: orgA.id, role: UserRole.ANALYST, updatedAt: new Date() },
    });
    admin = await db.user.create({
      data: { id: randomUUID(), email: `exec-admin-${stamp}@example.com`, name: "Admin", organizationId: orgA.id, role: UserRole.ADMINISTRATOR, updatedAt: new Date() },
    });
    analystB = await db.user.create({
      data: { id: randomUUID(), email: `exec-b-${stamp}@example.com`, name: "Analyst B", organizationId: orgB.id, role: UserRole.ANALYST, updatedAt: new Date() },
    });
