...
export const PERMISSION_MATRIX: Record<UserRole, Permission[]> = {
   ...
   * - Activates/deactivates compliance frameworks
   * - Read access to all data for oversight
   * - Does NOT manage day-to-day GRC workflows (that's GRC_ANALYST)
   */
  [UserRole.ORG_ADMIN]: [
    ...
    Permission.EVIDENCE_REQUEST_CANCEL, // Story 3.12
  ...
  /**
   * GRC_MANAGER: GRC Team Manager
   * - Creates and assigns assessment tasks
   * - Manages GRC analyst workload
   * - Full CRUD on Evidence and Risk (like GRC_ANALYST)
   * - Dashboard access for team oversight
   */
  ...
  /**
   * GRC_ANALYST: Primary GRC workflow operator
   * - Full CRUD on Evidence and Risk
   * - Assigns risks to IT/Business Stakeholders
   * - Primary user of compliance dashboards
  ...
  /**
   * SECURITY_ENGINEER: Technical risk creator
   * - Creates risks from security findings
   * - Can update risks they created (ownership model)
   * - Read access to evidence for context
   * - Read access to frameworks for mapping
   */
  [UserRole.SECURITY_ENGINEER]: [
    Permission.RISK_CREATE,
    Permission.RISK_READ_OWN, // Only risks they created
    Permission.RISK_UPDATE_OWN, // Only risks they created
    Permission.EVIDENCE_READ,
    Permission.EVIDENCE_CREATE, // Story 3.12: Upload evidence for requests
    Permission.EVIDENCE_REQUEST_READ, // Story 3.12: See requests assigned to them
    Permission.EVIDENCE_REQUEST_FULFILL, // Story 3.12: Upload evidence for requests
    Permission.FRAMEWORK_READ,
  ],
   ...
   * - No write access (delegates to GRC_ANALYST)
   */
  [UserRole.CISO]: [
    Permission.EVIDENCE_READ,
    Permission.EVIDENCE_REQUEST_READ, // Story 3.12: See requests (oversight)
    Permission.RISK_READ_ALL,
    Permission.FRAMEWORK_READ,
    Permission.DASHBOARD_COMPLIANCE,
    Permission.DASHBOARD_RISK,
  ...
  /**
   * IT_STAKEHOLDER: Remediation executor (IT teams)
   * - Sees only risks assigned to them
   * - Updates risk status and comments
   * - Uploads remediation evidence
   * - Very limited scope (only their work items)
   */
  [UserRole.IT_STAKEHOLDER]: [
    Permission.RISK_READ_OWN, // Only risks assigned to them
    Permission.RISK_UPDATE_OWN, // Update status on assigned risks
    Permission.EVIDENCE_CREATE, // Upload remediation evidence
    Permission.EVIDENCE_READ,
    Permission.EVIDENCE_REQUEST_READ, // Story 3.12: See requests assigned to them
    Permission.EVIDENCE_REQUEST_FULFILL, // Story 3.12: Upload evidence for requests
  ],
  ...
  /**
   * BUSINESS_STAKEHOLDER: Business decision maker
   * - Sees only risks requiring their decision
   * - Approves/rejects remediation plans
   * - Very limited scope (approval workflow only)
   */
  [UserRole.BUSINESS_STAKEHOLDER]: [
    Permission.RISK_READ_OWN, // Only risks requiring their decision
    Permission.RISK_UPDATE_OWN, // Approve/reject remediation
    Permission.EVIDENCE_REQUEST_READ, // Story 3.12: See requests assigned to them
    Permission.EVIDENCE_REQUEST_FULFILL, // Story 3.12: Upload evidence for requests
  ],
  ...
  /**
   * AUDITOR: External compliance auditor
   * - Read-only access (independence requirement)
   * - Access to evidence and risks
   * - Filtered by framework (e.g., only SOC2 data)
   * - Can export evidence packages
   * - No write access
   */
  [UserRole.AUDITOR]: [
    Permission.EVIDENCE_READ, // Read-only, filtered by framework
    Permission.RISK_READ_ALL,
    Permission.FRAMEWORK_READ,
    Permission.EXPORT_PDF, // Export evidence packages
  ],
};
 ...
 * @example
 * ```typescript
 * const permissions = getPermissionsForRole(UserRole.GRC_ANALYST);
